...
function ShipOrderDialogContent({
	address,
	addressZoneId,
	onOpenChange,
	onSuccess,
	orderId,
	orderNumber,
}: Omit<ShipOrderDialogProps, "open">) {
	const [selectedZoneId, setSelectedZoneId] = useState(addressZoneId);
...
export default function ShipOrderDialog({
	...
	return (
		<Dialog onOpenChange={onOpenChange} open={open}>
			{open ? (
				<ShipOrderDialogContent
					address={address}
					addressZoneId={addressZoneId}
					key={`${orderId}:${addressZoneId ?? ""}`}
