			...
			<Show
				...
				fallback={
					<>
						...
						<p class="px-4 text-black/60 text-xs sm:text-sm lg:text-base">Одоогоор бүтээгдэхүүн байхгүй байна</p>
					</>
				}
			>
				<h3 class="mb-2 font-black text-base sm:mb-2.5 sm:text-lg lg:text-xl">Үр дүн олдсонгүй</h3>
				<p class="mb-4 px-4 text-black/60 text-xs sm:mb-5 sm:text-sm lg:mb-6 lg:text-base">Таны шүүлтүүрт тохирох бүтээгдэхүүн олдсонгүй. Шүүлтүүрээ өөрчилж үзнэ үү.</p>
				<button type="button" onClick={props.onClearFilters} class="mx-auto min-h-[44px] border-2 border-black bg-primary px-4 py-2.5 font-bold text-xs uppercase shadow-[2px_2px_0_0_#000] transition-all active:translate-x-px active:translate-y-px active:shadow-[1px_1px_0_0_#000] sm:px-5 sm:py-3 sm:text-sm lg:px-6 lg:py-3.5 lg:text-base">
...
export function ProductErrorState() {
	return (
		<div class="py-8 text-center sm:py-10">
			...
			<p class="mt-1 text-black/60 text-xs sm:text-sm">Дахин оролдох уу?</p>
		</div>
	);
}
