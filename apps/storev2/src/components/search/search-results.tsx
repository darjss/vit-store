...
function SearchResultsError(props: { onRetry: () => void }) {
	return (
...
function SearchResultsEmpty(props: { searchQuery: string }) {
	return (
		<div class="enter-fade">
			<div class="flex flex-col items-center justify-center py-8 text-center">
				...
				<p class="text-muted-foreground/80 mt-1 text-sm">Доорх ангилалуудаас сонгож үзнэ үү</p>
			</div>
			<PopularCategories />
		</div>
	);
...
type SearchHitData = NonNullable<ReturnType<ReturnType<typeof useSearchStorefront>["data"]>>;

function SearchResultsHits(props: {
	data: SearchHitData;
	hasNavigationResults: boolean;
	onProductClick?: () => void;
	onProductSelect: (productId: number, productName: string, position: number) => void;
	searchQuery: string;
}) {
	return (
		<div>
			<Show when={props.hasNavigationResults}>
				<div class="enter-fade mb-4 space-y-3" style={{ "transition-duration": "250ms" }}>
					<Show when={(props.data.brands.length ?? 0) > 0}>
						<div>
							<p class="text-muted-foreground/80 mb-2 text-[11px] font-semibold tracking-wide uppercase">
								Брэнд
							</p>
							<div class="flex flex-wrap gap-2">
								<For each={props.data.brands ?? []}>
									{(brand) => (
										<a
					...
					<Show when={(props.data.categories.length ?? 0) > 0}>
						<div>
							<p class="text-muted-foreground/80 mb-2 text-[11px] font-semibold tracking-wide uppercase">
								Ангилал
							</p>
			...
			<div class="grid grid-cols-2 gap-2.5 sm:grid-cols-3 sm:gap-4 lg:grid-cols-4">
				<For each={props.data.products ?? []}>
					{(product, index) => (
						...
						>
							<ProductCard
								onInteract={() => props.onProductSelect(product.id, product.name, index())}
								product={product}
							/>
						</div>
					)}
		...
		<div class="mt-4 sm:mt-6">
			<Switch>
				...
				<Match when={search.isError()}>
					<SearchResultsError onRetry={() => search.refetch()} />
				</Match>

				<Match
					when={search.data() && search.data()?.products.length === 0 && !hasNavigationResults()}
				>
					<SearchResultsEmpty searchQuery={props.searchQuery} />
				</Match>
				...
				>
					<SearchResultsHits
						data={search.data()!}
						hasNavigationResults={hasNavigationResults()}
