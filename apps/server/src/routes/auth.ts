...
function isValidGoogleIdTokenAudience(audience: string | string[] | undefined, clientId: string): boolean {
    if (!clientId) {
        return false;
    }
...
function isValidGoogleIdTokenClaims(claims: GoogleIdTokenClaims, clientId: string): boolean {
    const validIssuer = claims.iss === "https://accounts.google.com" ||
...
app.get("/login/google", (c) => {
    ...
    setCookie(c, OAUTH_TEMP_COOKIE, JSON.stringify({ state, codeVerifier }), getOAuthCookieOptions(isSecure));
...
app.get("/login/google/callback", async (c) => {
    ...
    try {
        ...
        return c.redirect(`${c.env.DASH_URL}/login?message=` +
            encodeURIComponent("Таны бүртгэл баталгаажуулалтаар хүлээгдэж байна. Администратораас батламж авна уу."));
    }
    catch (e) {
        log.error(e instanceof Error ? e : new Error(String(e)), {
