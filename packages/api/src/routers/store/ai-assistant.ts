...
import type { StoreAssistantDisplayType, StoreAssistantPageContext, StoreAssistantResponse, } from "~/lib/types";
...
function mapStockStatus(status: string, stock: number): AssistantSearchToolResult["stockStatus"] {
...
async function getAssistantSearchProducts(input: {
    ...
    const canonicalProducts = await productQueries.store.getProductsByIdsWithDetails(ids);
...
async function retrieveAssistantContext(input: {
    ...
    ].filter((value): value is string => Boolean(value && value.length > 0)))).slice(0, 3);
    ...
    const detailedProducts = new Map<number, Awaited<ReturnType<typeof getAssistantProductInfo>>>();
    if (input.pageContext?.productId) {
        const pageProduct = await getAssistantProductInfo(input.pageContext.productId);
        if (pageProduct) {
    ...
    return {
        ...
        detailedProducts: [...detailedProducts.values()].filter((product): product is NonNullable<typeof product> => product !== null),
    };
}
function buildCatalogContext(products: Awaited<ReturnType<typeof retrieveAssistantContext>>["detailedProducts"]) {
...
function buildConversationPrompt(messages: Array<{
    role: "user" | "assistant";
    content: string;
}>, pageContext?: StoreAssistantPageContext) {
...
function normalizeAssistantResponse(response: z.infer<typeof assistantResponseSchema>, availableProductIds: Set<number>): StoreAssistantResponse {
    const uniqueProductIds = Array.from(new Set(response.productIds.filter((productId) => availableProductIds.has(productId)))).slice(0, 5);
...
export const aiAssistant = router({
    chat: publicProcedure
        .input(v.object({
        messages: v.pipe(v.array(messageSchema), v.minLength(1), v.maxLength(20)),
        locale: v.optional(v.string()),
        pageContext: v.optional(pageContextSchema),
    }))
        .mutation(async ({ input, ctx }) => {
        try {
            const prompt = buildConversationPrompt(input.messages, input.pageContext);
            const retrieval = await retrieveAssistantContext({
                messages: input.messages,
                pageContext: input.pageContext,
            });
            const availableProductIds = new Set(retrieval.detailedProducts.map((product) => product.id));
...
${catalogContext}`,
            ...
            ctx.log.error(error instanceof Error ? error : new Error(String(error)), {
                event: "store.aiAssistant.chat_failed",
