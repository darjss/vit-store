...
import { assertCanAccessOrder, type CustomerSessionClaims, createCheckoutAccessToken, } from "~/lib/session/checkout-access";
...
import { createAndStoreQpayInvoice } from "~/routers/store/payment";
        ...
        try {
            ...
            if (paymentNumber) {
                ctx.c.executionCtx.waitUntil(
                    createAndStoreQpayInvoice(paymentNumber).catch(() => {
                        ctx.log.warn("payment.qpay_eager_create_failed", {
                            paymentNumber,
                        });
                    }),
                );
