...
import { brandQueries, categoryQueries, productQueries, purchaseQueries, } from "@vit/api/queries";
import { type addPurchaseType, extractPurchaseFromImagesSchema, type extractPurchaseFromImagesType, saveExtractedPurchaseSchema, type saveExtractedPurchaseType, } from "@vit/shared/schema";
...
function scoreProductMatch(description: string, productName: string) {
    ...
    const containsBoost = normalizeText(productName).includes(normalizeText(description)) ||
        normalizeText(description).includes(normalizeText(productName))
        ? 0.2
        : 0;
    return Math.min(jaccard + containsBoost, 1);
}
function stringIncludesNeedle(haystack: string | null | undefined, needle: string | null | undefined) {
...
function scoreRetrievedCandidate(item: z.infer<typeof invoiceExtractionSchema>["items"][number], candidate: CandidateProduct) {
    ...
    if (stringIncludesNeedle(candidate.name, item.sourceCode))
        score += 0.24;
    if (normalizeText(candidate.name) === normalizeText(item.description) &&
...
async function inferInvoiceData(input: extractPurchaseFromImagesType, brands: {
    id: number;
    name: string;
}[], categories: {
    id: number;
...
`,
    ...
    for (const [index, rankedCandidates] of rankedCandidatesByIndex.entries()) {
        ...
        if (!clearlyMatched) {
            ambiguousCandidates.set(index, rankedCandidates.map((entry) => entry.candidate));
        }
    }
    ...
    return {
        ...
        items: dedupedItems.map((item, index) => {
            ...
            const exactBrand = brands.find((brand) => normalizeText(brand.name) === normalizeText(item.brand));
            const category = categories.find((entry) => normalizeText(entry.name) === normalizeText(item.categoryGuess));
            ...
            const autoMatched = matched &&
                ...
                ? matched.candidate
                : null;
            const aiMatched = !autoMatched && aiRerank?.bestCandidateId
                ? (rankedCandidates.find((entry) => entry.candidate.id === aiRerank.bestCandidateId)?.candidate ?? null)
                : null;
...
async function ensureBrandId(tx: Parameters<Parameters<ReturnType<typeof db>["transaction"]>[0]>[0], brandId: number | null | undefined, brandName: string | null | undefined) {
    if (brandId)
        return brandId;
    if (!brandName?.trim())
        return null;
    ...
    const created = await tx
        ...
        .values({
        name: brandName.trim(),
        slug: createSlug(brandName.trim()),
...
async function createProductFromDraft(tx: Parameters<Parameters<ReturnType<typeof db>["transaction"]>[0]>[0], item: saveExtractedPurchaseType["items"][number]) {
    if (item.productId)
    ...
    if (draft.images?.length) {
        await tx.insert(ProductImagesTable).values(draft.images.map((image, index) => ({
            productId,
            url: image.url,
            isPrimary: index === 0,
        })));
