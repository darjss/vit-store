import { TRPCError } from "@trpc/server";
import { brandQueries } from "@vit/api/queries";
import { addBrandSchema, brandTag } from "@vit/shared";
import * as v from "valibot";
import { purgeCatalogCache } from "~/lib/cache/workers-cache";
import { scheduleProductSearchRebuild } from "~/lib/product-search/client";
import { slugify } from "~/lib/utils";
import { adminProcedure, baseProcedure, botProcedure, router } from "~/lib/trpc";
export function buildBrandsRouter<P extends typeof baseProcedure>(proc: P) {
	return router({
		addBrand: proc.input(addBrandSchema).mutation(async ({ ctx, input }) => {
			try {
				const slug = input.slug || slugify(input.name);
				const { id: _id, ...data } = input;
				await brandQueries.admin.createBrand({
					...data,
					slug,
				});
				await purgeCatalogCache(ctx);
				scheduleProductSearchRebuild(ctx, "brand_updated");
				return { message: "Successfully updated category" };
			} catch (error) {
				ctx.log.error(error instanceof Error ? error : new Error(String(error)), {
					event: "addBrand",
				});
				throw new TRPCError({
					cause: error,
					code: "INTERNAL_SERVER_ERROR",
					message: "Failed to add products",
				});
			}
		}),
		deleteBrand: proc.input(v.object({ id: v.number() })).mutation(async ({ ctx, input }) => {
			try {
				await brandQueries.admin.deleteBrand(input.id);
				await purgeCatalogCache(ctx, [], [brandTag(input.id)]);
				scheduleProductSearchRebuild(ctx, "brand_updated");
			} catch (error) {
				ctx.log.error(error instanceof Error ? error : new Error(String(error)), {
					event: "deleteBrand",
				});
				throw new TRPCError({
					cause: error,
					code: "INTERNAL_SERVER_ERROR",
					message: "Failed to delete brand",
				});
			}
		}),
		getAllBrands: proc.query(async ({ ctx }) => {
			try {
				const brands = await brandQueries.admin.getAllBrands();
				ctx.log.info("getAllBrands", { count: brands.length });
				return brands;
			} catch (error) {
				ctx.log.error(error instanceof Error ? error : new Error(String(error)), {
					event: "getAllBrands",
				});
				throw new TRPCError({
					cause: error,
					code: "INTERNAL_SERVER_ERROR",
					message: "Error fetching brands",
				});
			}
		}),
		updateBrand: proc.input(addBrandSchema).mutation(async ({ ctx, input }) => {
			try {
				const id = input.id;
				if (!id) {
					throw new TRPCError({
						code: "INTERNAL_SERVER_ERROR",
						message: "Failed to add products",
					});
				}
				const slug = input.slug || slugify(input.name);
				const { id: _id, ...data } = input;
				await brandQueries.admin.updateBrand(id, {
					...data,
					slug,
				});
				await purgeCatalogCache(ctx, [], [brandTag(id)]);
				scheduleProductSearchRebuild(ctx, "brand_updated");
			} catch (error) {
				ctx.log.error(error instanceof Error ? error : new Error(String(error)), {
					event: "updateBrand",
				});
				throw new TRPCError({
					cause: error,
					code: "INTERNAL_SERVER_ERROR",
					message: "Failed to add products",
				});
			}
		}),
	});
}
export const brands = buildBrandsRouter(adminProcedure);
export const brandsBot = buildBrandsRouter(botProcedure);
