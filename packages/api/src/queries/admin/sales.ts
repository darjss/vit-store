...
type AdminSalesRepository = {
	addSale: (sale: AddSalesType) => Promise<unknown>;
	getAnalyticsForHome: (
		timeRange?: "daily" | "weekly" | "monthly",
	) => Promise<{ revenue: number; salesCount: number; profit: number }>;
	getMostSoldProducts: (
	...
	getRevenue: (
		timeRange: timeRangeType,
	) => Promise<Array<{ revenue: number | null }>>;
};

export function adminSales(db: DB): AdminSalesRepository {
	return {
