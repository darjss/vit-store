...
export async function runCacheBenchmarkV2(options?: { iterations?: number; warmup?: number }) {
	...
	const dbTimes: Array<number> = [];
	const kvReadHitTimes: Array<number> = [];
	const kvWriteTimes: Array<number> = [];
	const redisReadHitTimes: Array<number> = [];
	const redisWriteTimes: Array<number> = [];
	...
	for (let i = 0; i < iterations; i++) {
		...
		redisWriteTimes.push(
			await measureMs(async () => {
				await redis().set(`benchmark:v2:${benchmarkId}:redis:write:${i}`, payload, { ex: 3600 });
			}),
		);
	}

	const kvCompositeMean =
		summarizeTimings(kvReadHitTimes).mean + summarizeTimings(kvWriteTimes).mean;
	const redisCompositeMean =
		summarizeTimings(redisReadHitTimes).mean + summarizeTimings(redisWriteTimes).mean;
