...
export async function readSession(sessionId: string): Promise<AiProductSessionState> {
	const session = await kv().get<AiProductSessionState>(aiProductSessionKey(sessionId), "json");
	if (!session) {
		throw new TRPCError({
			code: "NOT_FOUND",
...
export async function writeSession(sessionId: string, state: AiProductSessionState): Promise<void> {
