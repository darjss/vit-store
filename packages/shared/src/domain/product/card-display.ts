...
const FORM_DEFINITIONS: Array<{ label: string; pattern: RegExp }> = [
	{
		label: "Зөөлөн капсул",
		pattern: /(?:mini[-\s]*)?(?:veggie\s+)?soft[-\s]*gels?|softgel\s+capsules?/iu,
	},
	{
		label: "Капсул",
		pattern: /(?:(?:veg(?:gie|an)?|vegetarian|dr)\s+)?(?:capsules?|caps?)|v-?caps/iu,
	},
	{
		label: "Шахмал",
	...
	{
		label: "Гами",
		pattern: /(?:pectin-based(?:\s+[\w-]+){0,2}\s+)?chews?|gummy\s+chews?|gummies?/iu,
	},
	{ label: "Хүлхдэг шахмал", pattern: /lozenges?/iu },
	{ label: "Үрэл", pattern: /pills?/iu },
	{ label: "Дусал", pattern: /drops?/iu },
	{ label: "Уут", pattern: /(?:stick\s+packs?|packets?|sachets?)/iu },
	{ label: "Гель", pattern: /gels?/iu },
...
const removePrefix = (name: string, prefix: string) => {
	...
	for (let index = 0; index < name.length; index += 1) {
		...
		if (!prefixKey.startsWith(currentKey)) {
			return;
		}
		if (currentKey === prefixKey) {
...
const normalizePackageQuantity = (value: string) =>
	clean(
		value
			.replaceAll(/(\d)(fl\s*oz|oz|lb|lbs|kg|g|ml|l)\b/giu, "$1 $2")
			.replaceAll(/\s*\(\s*/gu, " (")
			.replaceAll(/\s*\)\s*/gu, ")"),
	);

const findStandaloneForm = (value: string) => {
	const definition = FORM_DEFINITIONS.find(({ pattern }) => pattern.test(value));
	if (definition) {
		return definition.label;
	}
	if (/powders?|нунтаг/iu.test(value)) {
		return "Нунтаг";
	}
	if (/liquids?|oils?|solutions?|шингэн/iu.test(value)) {
		return "Шингэн";
	}
};

const inferPackageForm = (amount?: string | null) =>
	amount && /\b(?:fl\s*oz|milliliters?|ml|liters?|l)\b/iu.test(amount) ? "Шингэн" : undefined;

const findItemAmount = (amount: string) => {
	for (const definition of FORM_DEFINITIONS) {
		const match = amount.match(
			new RegExp(String.raw`\b(\d[\d,.]*)\s+(${definition.pattern.source})\b`, "iu"),
		);
		if (match) {
	...
	return shorthand ? { count: shorthand[1], form: "Ширхэг", matched: shorthand[0] } : undefined;
};

const packageRemainder = (amount: string, matchedItem?: string) => {
	if (!matchedItem) {
		return normalizePackageQuantity(amount) || undefined;
	}
	const remainder = clean(amount.replace(new RegExp(escapeRegExp(matchedItem), "iu"), ""));
...
const conciseDose = (potency: string) => {
	const tokens = (potency.match(DOSE_TOKEN_PATTERN) ?? [])
		.map((token) => token.replaceAll(/\s+/gu, " ").trim())
		.slice(0, 3);
	return tokens.length > 0 ? bounded(tokens.join(" + "), MAX_DOSE) : undefined;
};

const removeExact = (value: string, phrase?: string | null) =>
	phrase?.trim() ? value.replaceAll(new RegExp(escapeRegExp(phrase.trim()), "giu"), "") : value;

const stripFormVocabulary = (value: string) => {
	let result = value;
	for (const definition of FORM_DEFINITIONS) {
		result = result.replaceAll(new RegExp(definition.pattern.source, "giu"), "");
	}
	return result
		.replaceAll(/\b(?:powders?|liquids?|нунтаг|шингэн)\b/giu, "")
		.replaceAll(/\b\d+s\b/giu, "");
};
